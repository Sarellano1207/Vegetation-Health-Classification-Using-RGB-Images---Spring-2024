...
def flatten_dataset(data_set: xr.DataArray) -> np.ndarray:
    ...
    """
    data_arrays = data_set.to_array(dim="tile")
    new_array = data_arrays.values.reshape(data_arrays.shape[0] * data_arrays.shape[1] * data_arrays.shape[2] * data_arrays.shape[3] * data_arrays.shape[4])
    return new_array
...
def flatten_dataset_by_band(data_set: xr.Dataset) -> np.ndarray:
    ...
    """
    data_arrays = data_set.to_array(dim="tile")
    new_array = data_arrays.values.reshape(data_arrays.shape[2], data_arrays.shape[0] * data_arrays.shape[1] * data_arrays.shape[3] * data_arrays.shape[4])
    return new_array
...
def plot_sentinel2_histogram(
    ...
    # --- start here ---
    # iterate by band
    for band in range(len(satellite_bands)):
        if(np.isfinite(flattened_dataset_list[band]).all()):
            # set scale to log
...
def plot_gt(data_array: xr.DataArray, image_dir: Path = None) -> None:
    ...
    ax[0,0].imshow(data_array.squeeze())
...
def plot_max_projection_viirs(data_array: xr.DataArray, image_dir: Path = None) -> None:
    ...
    # show the image
    plot_array = data_array.squeeze()
    ax[0,0].imshow(plot_array)
    ...
    for row in range(num_rows):
        ...
        for col in range(num_cols):
            ...
            if date_index < num_dates:
                # show the image
                ax[row, col].imshow(data_array[date_index, band].squeeze(), cmap="viridis")
...
def plot_satellite_by_bands(
    ...
    for index, bands in enumerate(bands_to_plot):
        ...
        for date in range(num_dates):
            ...
            ax[date, index].imshow(np.stack(data).transpose())
            # set the axis to off
